package com.sun.ev_dictionary.utils

object Constants {
    const val FILE_NAME_EV = "ev.txt"
    const val EN_WORD_REGEX_TYPE_1 = "###-"
    const val EN_WORD_REGEX_TYPE_2 = "###*"
    const val EN_WORD_REGEX_TYPE_3_START = "##"
    const val EN_WORD_REGEX_TYPE_3_END = "#*"
    const val EN_WORD_REGEX_TYPE_4_END = "#-"
    const val EN_WORD_REGEX_TYPE_5_START = "="
    const val EN_WORD_REGEX_TYPE_5_END = "###"
    const val EN_WORD_REGEX_ENDLINE_1 = "|- "
    const val EN_WORD_REGEX_ENDLINE_2 = "|*"
    const val EN_WORD_REGEX_ENDLINE_3 = "|"
    const val EN_WORD_REGEX_EG_EN = "|="
    const val EN_WORD_REGEX_EG_VI = "|+"
    const val WORD_REPLACE_ENDLINE = "\n"
    const val EN_WORD_REPLACE_EG_EN = "\nE.g. "
    const val EN_WORD_REPLACE_EG_VI = "\n=>"
    const val PREF_ENGLISH_WORDS = "PREF_ENGLISH_WORDS"
}
